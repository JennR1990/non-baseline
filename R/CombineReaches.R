GetUnbaselinedData<- function(experiment) {
  ...
  for (participant in participants){
    ...
    } else if (experiment %in% c(8)) {
      print(nrow(expangles))
      expangles[,sprintf('p%d',participant)] <- baselinedangles$reaches[1:48]
    }
  }
  outputfilename<- sprintf('../../non-baseline/data/time_model%d_Reaches.csv', experiment)
...
baselinebyaligned<- function(df, experiment, dist) {
  ...
  if (experiment == 1| experiment == 5| experiment == 7) {
    ...
    #Subtract the bias if you actually want to baseline data
    ...
    bias<-mean(df$reachdeviations[1:48], na.rm = TRUE)
    reaches <- reaches- 0
  } else {
    print('hi')
  }
