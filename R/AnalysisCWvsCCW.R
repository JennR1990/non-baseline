...
TUnbaselinedCombine<- function(data) {
  ...
  for (participant in participants){
    Aligned<- mean(unlist(data[57:64,participant]), na.rm = TRUE)
    ...
    EC<- mean(unlist(data[241:248,participant]), na.rm = TRUE)
    EC_Late<- mean(unlist(data[281:288,participant]), na.rm = TRUE)
